...
<template>
  <section :aria-labelledby="headingId" class="bg-white w-full">
    ...
    <div
      :id="headingId"
      class="px-4 py-2 bg-surface-container-low text-primary flex items-center justify-between transition-colors cursor-pointer select-none"
    ...
    <!-- Loading skeleton -->
    <div v-if="loading && !collapsed" class="divide-y divide-outline-variant/10">
    ...
    <!-- Error -->
    <p v-else-if="error && !collapsed" class="px-6 py-4 text-sm text-error">{{ error }}</p>

    <!-- Empty -->
    <p v-else-if="items.length === 0 && !collapsed" class="px-6 py-4 text-sm text-on-surface-variant italic">
      No appointments
    </p>

    <!-- Cards -->
    <div v-else-if="!collapsed" class="divide-y divide-outline-variant/10">
      <SwipeCard
        v-for="appt in items"
