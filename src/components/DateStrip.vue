...
const dates = computed(() => {
  ...
  const start = props.includeToday ? 0 : 1
  for (let i = start; i <= start + 13; i++) {
    const d = new Date(base)
    d.setDate(base.getDate() + i)
    result.push({ d, ds: toDateStr(d), disabled: props.disabledDaysOfWeek.includes(d.getDay()) })
  }
  return result
})
