...
<template>
  ...
  >
    <template #header>
      ...
      <div v-if="loading" class="grid grid-cols-2 gap-3 p-4" aria-busy="true" aria-label="Loading price list">
        <div v-for="n in 6" :key="n" class="overflow-hidden rounded-2xl border border-outline-variant/20 bg-surface">
          <div class="aspect-[4/3] animate-pulse bg-surface-container" />
          <div class="space-y-2 p-3">
            <div class="h-4 w-4/5 animate-pulse rounded bg-surface-container" />
            <div class="h-3 w-3/5 animate-pulse rounded bg-surface-container" />
            <div class="h-5 w-2/5 animate-pulse rounded bg-surface-container" />
          </div>
        </div>
      </div>
      ...
      <div v-else class="space-y-4 p-4">
        <section v-for="group in rendered.groups" :key="group.category">
          <div
            class="sticky top-0 z-10 -mx-4 mb-3 flex items-center justify-between border-y border-outline-variant/20 bg-surface-container-low/95 px-4 py-2 backdrop-blur-sm"
          >
