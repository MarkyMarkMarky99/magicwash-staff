...
const totalWeightKg = computed(() => orderTotalWeightKg(images.value))
...
let weightCaptureSubmitted = false
...
const registrationItem = computed(() => currentOrder.value?.orderId === orderId.value
  ...
  : null)
const registrationUnassigned = computed(() => isRegistrationOpen.value && registrationItemId.value === null)
const registrationReady = computed(() => registrationItem.value !== null || registrationUnassigned.value)
const registrationLabel = computed(() => registrationItem.value?.description || (registrationUnassigned.value ? 'garments (no item)' : 'Garment'))
...
const registrationCanSave = computed(() =>
  isRegistrationOpen.value && registrationReady.value && registrationTagsReady.value && !registrationLoading.value && canSaveGarment(
...
function consumeOrderApprovalNotice(): void {
  ...
  if (!state || !Object.hasOwn(state, 'orderApprovalNotice')) return
  const notice: unknown = state.orderApprovalNotice
  const nextState = { ...state }
  delete nextState.orderApprovalNotice
  window.history.replaceState(nextState, '')
  if (!notice || typeof notice !== 'object') return
  if (!('orderId' in notice) || notice.orderId !== orderId.value) return
  if (!('message' in notice) || typeof notice.message !== 'string') return
  if (!('success' in notice) || typeof notice.success !== 'boolean') return
  orderApprovalNotice.value = { message: notice.message, success: notice.success }
}
...
function openUnassignedRegistration(): void {
  if (!orderId.value) return
  void primeFeedbackAudio()
  orderOverlay.open('register-garment')
}

function acceptRegistrationTag(value: string): void {
  if (!registrationReady.value) return
...
function saveRegistration(): void {
  if (!registrationCanSave.value || !registrationTag.value || !registrationPhoto.value) return
...
function openPhotoLibrary(): void {
  if (!orderId.value) return
  void router.push({ name: 'order-photo-library', params: { orderId: orderId.value } })
}
...
async function handleCapture(file: File): Promise<void> {
  ...
  if (imageType === 'WEIGHT') {
    if (weightCaptureSubmitted) return
