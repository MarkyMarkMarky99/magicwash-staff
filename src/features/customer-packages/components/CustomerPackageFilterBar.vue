...
const packageStatuses = [null, ...customerPackageStatusSchema.options] as const
</script>
...
<template>
  <section class="space-y-3 px-4 py-3">
    ...
    <div class="flex flex-wrap gap-2">
      <button v-for="status in packageStatuses" :key="status ?? 'all'" type="button" class="rounded-full px-3 py-1 font-label text-xs" :class="filter.status === status ? 'bg-primary text-on-primary' : 'bg-surface-container text-on-surface-variant'" @click="emit('change', { status })">{{ status ?? 'All' }}</button>
    </div>
  </section>
</template>
