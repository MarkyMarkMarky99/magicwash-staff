...
await withMockSheets(
  async (call) => {
    ...
    if (call.init?.method === 'POST' && path.endsWith('/values/PriceList!A:N:append')) {
  ...
  async (calls) => {
    ...
    const appendCall = calls.find((call) => apiPath(call.url).endsWith('/values/PriceList!A:N:append'))
