...
process.env.AFTER_PHOTOS_SPREADSHEET_ID = 'after-photos-test-spreadsheet'
const { privateKey } = generateKeyPairSync('rsa', { modulusLength: 2048 })
process.env.GOOGLE_SERVICE_ACCOUNT_KEY = Buffer.from(JSON.stringify({
  client_email: 'after-photos@example.test',
  private_key: String(privateKey.export({ format: 'pem', type: 'pkcs8' })),
})).toString('base64')

const { createAfterPhotoRepository, afterPhotoRoutes } = await import('../../../../../server/modules/after-photos/after-photo.module.js')
const moduleSource = readFileSync(
  fileURLToPath(new URL('../../../../../server/modules/after-photos/after-photo.module.ts', import.meta.url)),
  'utf8',
)
assert.doesNotMatch(moduleSource, /formatBangkokTimestamp/)

const headers = [
  'id', 'order_id', 'orderitem_id', 'item_id', 'image_path', 'image_url', 'notes', 'created_at',
...
globalThis.fetch = (async (input: URL | string, init?: RequestInit) => {
  ...
  if (init?.method === 'GET' && path.endsWith('/values/after!1:1')) {
    return jsonResponse({ values: [headers] })
  }
  if (init?.method === 'POST' && path.endsWith('/values/after!A:A:append')) {
    ...
    return jsonResponse({
      updates: {
        ...
        updatedRange: `after!A${rows.length - body.values.length + 2}:P${rows.length + 1}`,
...
try {
  for (const invalid of [
    ...
    const result = await afterPhotoRoutes.collection.handleRequest(request('POST', invalid))
    assert.equal(result.status, 422)
  }
  assert.equal(rows.length, 0, 'invalid creates must not append')

  for (const forbiddenKey of [
    'id', 'afterPhotoId', 'createdAt', 'updatedAt', 'updatedBy', 'imagePath', 'notes',
    'checked', 'isActive', 'fileId', 'deletedAt', 'deletedBy',
  ] as const) {
    const result = await afterPhotoRoutes.collection.handleRequest(request('POST', {
      orderId: 'order-1', imageUrl: 'https://example.test/photo.jpg', createdBy: 'staff-1',
      [forbiddenKey]: 'client-value',
    }))
    assert.equal(result.status, 422)
  }
  assert.equal(rows.length, 0, 'forbidden create fields must not append')

  const created = await afterPhotoRoutes.collection.handleRequest(request('POST', {
    orderId: ' order-1 ',
  ...
  const createdWithoutItems = await afterPhotoRoutes.collection.handleRequest(request('POST', {
  ...
  const repository = createAfterPhotoRepository()
...
console.log('after-photo writes dry test passed')
