...
): { service: AfterPhotoService; photos: FakePhotoRepository; items: FakeItemRepository } {
  const photos = makePhotoRepository(photoRows)
  const items = makeItemRepository(itemRows)
  return {
    service: new AfterPhotoService({
      repository: photos,
...

{
  ...
  await expectApiError(
    () => service.update('after-1', { orderItemId: 'missing-item', updatedBy: 'staff-1' }),
    404,
  )
...

{
  ...
  await expectApiError(
    () => service.update('after-1', { orderItemId: 'destination-item', updatedBy: 'staff-1' }),
    400,
  )
  assert.equal(photos.updateCalls.length, 0)
}
...
console.log('after-photo service dry test passed')
