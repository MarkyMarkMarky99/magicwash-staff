...
globalThis.fetch = (async (input: URL | string, init?: RequestInit) => {
  ...
  if (init?.method === 'POST' && path.endsWith('/values/Packages!A:L:append')) {
    ...
    return jsonResponse({
      ...
      updates: { updatedRows: 1, updatedRange: 'Packages!A2:L2', updatedData: { values: body.values } },
    })
  }
...
try {
  ...
  const appendBody = JSON.parse(String(calls.find((call) => call.init?.method === 'POST' && pathOf(call.url).endsWith('/values/Packages!A:L:append'))?.init?.body)) as { values: PackageRow[] }
