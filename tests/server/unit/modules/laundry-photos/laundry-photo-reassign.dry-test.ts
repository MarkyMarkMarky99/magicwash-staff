...
function setup(itemRows: Array<Partial<ItemRow>> = [item], reject = false) {
  ...
  const photoRepository = {
    ...
    async updateMany(updates: ReadonlyArray<SheetRowUpdate<PhotoRow>>): Promise<PhotoRow[]> {
      ...
      return updates.map(({ keyValue, patch }) => ({
        ...
        image_path: null, image_url: null, notes: null, timestamp: null, created_by: null,
  ...
  return {
    service: new LaundryPhotoService({ repository: photoRepository, orderItemFormsRepository: () => itemRepository }),
...
const payload = { photoIds: [' photo-2 ', ' photo-1 '], orderItemId: ' item-2 ', updatedBy: ' staff-1 ' }
{
  ...
  assert.deepEqual(result.photos.map(photo => photo.laundryPhotoId), ['photo-2', 'photo-1'])
...
console.log('laundry-photo reassign dry test passed')
