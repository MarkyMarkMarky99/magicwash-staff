 ...
 * `sku` as a valid nullable column, and an earlier draft of
 * `docs/contracts/invoice-write.contract.ts` still types it as a required-
 * but-nullable `string | null` key. The coordinator's chat instruction was
 * explicit and more recent: "`sku` is dropped from the form and the payload."
...
export const invoiceItemRowSchema = z
  .object({
     ...
     * not a per-entity format (settles open decision #1 in
     * `docs/contracts/invoice-write.contract.ts`). Generated by the future
     * service, not by this contract file.
     */
