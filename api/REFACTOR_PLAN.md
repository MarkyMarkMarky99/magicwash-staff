...
BaseRepository<TApiRow, TReadWhere, TCreate, TUpdateFilter, TUpdate, TDeleteFilter>
RepositoryReadQuery<TReadWhere>
...
read({ where: { id } }) -> Promise<Array<Partial<TApiRow>>>
read({ where, select: ['id'] }) -> Promise<Array<Partial<TApiRow>>>  // select projects -> Partial
...
GSheetRepository<TApiRow, TDbRow, TReadWhere, TCreate, TUpdateFilter, TUpdate, TDeleteFilter>
GSheetRepositoryOptions<TDbRow>
GVizFetchInput
...
update merge rule: where wins
AppScriptResponse<TDbRow>.data -> TDbRow  // stored DB row; mapper.toApi turns it into TApiRow

Current scope:
BaseRepository.delete(filter: TDeleteFilter) -> Promise<unknown>
GSheetRepository.delete(filter: TDeleteFilter) -> future implementation
...
TUpdateFilter: { id: string }
TDeleteFilter: { id: string }
...
const appointmentRepository =
  new GSheetRepository<
    ...
    AppointmentUpdateFilter, // TUpdateFilter — { id: string }
    AppointmentUpdateInput   // TUpdate       — API/domain update input
    // TDeleteFilter defaults to TUpdateFilter
  >({
