...
The result card shows a loading state while saving. `advanced` shows “รับงานแล้ว” or “เสร็จแล้ว” in success tone. `already_completed` shows a warning. `blocked`, `not_found`, `not_advanceable`, and `write_failed` show red explanations; uncertain writes ask staff to check before scanning again. Network and API errors show a retry message. Success vibrates briefly; other outcomes use a distinct vibration pattern. The shared scanner's built-in read vibration is disabled on this page only.
