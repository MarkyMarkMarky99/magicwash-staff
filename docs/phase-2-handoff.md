...
**✅ งานโค้ดของ Phase 2 จบครบแล้ว** — §2.9 (ทั้ง 4 ชีตย้ายมา Sheets API พิสูจน์กับ Google จริง
ครบ) · §2.10 (`154445b` ลบ SheetLib write path ทิ้ง เหลือทางเขียนทางเดียว) · `certainty`
(`78efaad`) · ด่าน preview ปิดครบทั้งอ่านและเขียน (`INV260832407923` สร้างบน preview จริง)

⬜ **เหลือ 2 อย่าง:** เจ้าของกดทดสอบ preview รอบสุดท้าย (หลัง §2.10) แล้ว **merge เข้า main**
· ข้อ 4 timestamp พักไว้ ไม่ใช่ตัวบล็อก · รายละเอียดหมวด 1c
...
### env ที่เหลือให้เจ้าของลบเอง (โค้ดไม่อ้างถึงแล้ว)

`APPSCRIPT_URL` · `APPSCRIPT_GATEWAY_URL` · `APPSCRIPT_APPOINTMENT_URL` — ไม่มี reference ใน
TypeScript เหลืออยู่เลย เป็นแค่รายการค้างใน `.env.local` และบน Vercel
**ห้ามลบ `APPSCRIPT_INVOICE_VIEW_SYNC_URL`** — ยังใช้จริงในการ recompute InvoicesView
...
### 📌 งานค้างที่เกือบหายไปกับ transcript — คอมเมนต์ "กอง B"
...
| B | **~35 คอมเมนต์ที่ "พูดซ้ำสิ่งที่ไฟล์อื่นเขียนไว้แล้ว"** | ⬜ **ยังไม่ทำ** |
| C | `api/CLAUDE.md` + `invoice-view-api.example.md` | ✅ แก้แล้ว |

**กอง B ถูกเลื่อนโดยตั้งใจ** เพราะเป็นงานใช้วิจารณญาณ ไม่ใช่ "ผิดเพราะเป็นสถานะ" แบบกอง A
⇒ ให้คนตัดสินทีละอัน ไม่ใช่สั่ง pipeline กวาด

🔴 **inventory เดิมใช้ไม่ได้แล้ว** — มันถูกทำขึ้นก่อนการรีไรต์ 36 ไฟล์ หลายรายการถูกเขียนใหม่ไป
แล้ว และ §2.10 เพิ่งลบโค้ดทิ้งอีกก้อนใหญ่ ⇒ **ต้องกวาดใหม่ ห้ามใช้รายการเก่า**
...
นั่นคือเหตุผลที่กองนี้ไม่ถูกโยนให้ agent ตั้งแต่แรก
...
⇒ ถ้าไม่มีใครถามหา มันจะหายไปพร้อม transcript
